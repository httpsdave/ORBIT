...
const openStatusModal = (app) => {
    ...
    feedbackText.value = app.feedback || '';
    ...
    if (typeof router !== 'undefined' && router.post) {
        router.post(`/applications/${selectedApplication.value.id}/update-status`, {
            ...
            onSuccess: () => {
                ...
                if (index !== -1) {
                    filteredApplications.value[index].status = newStatus.value;
                    filteredApplications.value[index].feedback = feedbackText.value;
                }
            ...
            onError: (errors) => {
                ...
                message.value = "Failed to update application status.";
                showMessage.value = true;
            },
            onFinish: () => {
        ...
        // Fall back to regular fetch with CSRF handling
        // Use the correct URL path - the endpoint may be wrong
        const url = `/applications/${selectedApplication.value.id}/update-status`;
        
        // Try to get the CSRF token through different methods
        let csrfToken = '';
        
        // Method 1: Look for the meta tag
        const csrfMeta = document.querySelector('meta[name="csrf-token"]');
        if (csrfMeta) {
            csrfToken = csrfMeta.getAttribute('content');
        } 
        // Method 2: Check for XSRF-TOKEN cookie
        ...
        .then(data => {
            ...
            // Show success message
            message.value = data.message || "Application status updated successfully!";
            showMessage.value = true;
...
<div v-if="showStatusModal && props.isAdmin" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    <!-- Backdrop -->
    <div class="absolute inset-0 bg-black bg-opacity-50 backdrop-blur-sm" @click="closeStatusModal"></div>
...
<!-- View Feedback Modal (For Regular Users) -->
<div v-if="showStatusModal && !props.isAdmin" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    <!-- Backdrop -->
    <div class="absolute inset-0 bg-black bg-opacity-50 backdrop-blur-sm" @click="closeStatusModal"></div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
        <!-- Modal Header with gradient -->
        <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
            <h3 class="text-xl font-bold">Application Feedback</h3>
            <p class="text-sm text-indigo-100 mt-1">
                {{ selectedApplication?.organization_name }}
            </p>
        </div>
        ...
        <div class="p-6">
            ...
            <div class="mb-4">
                ...
                <span :class="`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(selectedApplication?.status)}`">
