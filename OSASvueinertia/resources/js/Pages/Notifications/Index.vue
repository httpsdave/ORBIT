          ...
          <div class="p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <div class="flex items-center">
                ...
                <span v-if="unreadCount > 0" class="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  {{ unreadCount }} unread
                </span>
              </div>
              <button 
            ...
            <!-- Filters -->
            <div class="bg-gray-50 rounded-lg mb-6 border border-gray-100 overflow-hidden">
              <div class="p-4">
                <div class="flex flex-wrap items-end gap-4">
                  <div>
                    <label for="type-filter" class="block text-xs font-medium text-gray-500 mb-1">Filter by type</label>
                    <select 
                      id="type-filter" 
                      v-model="filters.type" 
                      @change="applyFilters"
                      class="block w-full pl-3 pr-10 py-2 text-sm border-gray-200 focus:outline-none focus:ring-blue-500 focus:border-blue-500 rounded-md"
                    >
                  ...
                  <div>
                    <label for="read-filter" class="block text-xs font-medium text-gray-500 mb-1">Filter by status</label>
                    <select 
                      id="read-filter" 
            ...
            <div v-else class="space-y-3">
              <button
                ...
                class="w-full block border rounded-lg overflow-hidden transition duration-200 ease-in-out hover:shadow focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                :class="[notification.is_read ? 'border-gray-100' : 'border-blue-200 shadow-sm', getNotificationClass(notification.type)]"
              >
                <div class="p-4">
                  <div class="flex">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex flex-col sm:flex-row sm:justify-between mb-1">
                        <h3 class="text-sm font-medium text-gray-900" :class="{'font-semibold': !notification.is_read}">
                          {{ notification.title }}
                          <span v-if="!notification.is_read" class="inline-block h-2 w-2 ml-2 rounded-full bg-blue-500"></span>
                        </h3>
                        <div class="flex items-center text-xs text-gray-500 mt-1 sm:mt-0">
