      ...
      <li v-for="event in todaysEvents" :key="'today-' + event.id" class="py-4">
        <div class="flex items-start space-x-4">
          ...
          <div v-if="isAdmin" class="flex-shrink-0 flex space-x-2">
            <button @click="$emit('edit-event', event)" class="text-blue-500 hover:text-blue-700 p-1 rounded transition-colors duration-200">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </button>
            <button @click="$emit('delete-event', event.id)" class="text-red-500 hover:text-red-700 p-1 rounded transition-colors duration-200">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default {
  ...
  emits: ['edit-event', 'delete-event', 'view-event-details'],
