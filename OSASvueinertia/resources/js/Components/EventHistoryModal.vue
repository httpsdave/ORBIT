      ...
      <div class="flex justify-between items-center p-6 border-b">
        ...
        <div class="flex items-center space-x-2">
          <button 
            ...
            class="inline-flex items-center justify-center p-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-xl shadow-md hover:from-green-600 hover:to-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-300 relative overflow-hidden group"
          ...
          <button @click="closeModal" class="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-100 transition-all duration-200">
          ...
          <li v-for="event in pastEvents" :key="'past-' + event.id" class="py-4">
            <div class="flex items-start space-x-4">
              <div :class="[
                ...
                <span :class="[
                  'text-sm font-medium',
                  event.status === 'cancelled' ? 'text-red-500' : 'text-gray-500'
                ...
                <span :class="[
                  'text-xs',
                  event.status === 'cancelled' ? 'text-red-400' : 'text-gray-400'
                ]">{{ formatDate(event.start_date, 'YYYY') }}</span>
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center space-x-2">
                  <p class="text-sm font-medium text-gray-700 truncate">{{ event.title }}</p>
                  <span v-if="event.status === 'cancelled'" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
              ...
              <div v-if="isAdmin" class="flex-shrink-0">
                <button @click="deletePastEvent(event.id)" class="text-red-500 hover:text-red-700 p-2 rounded-lg hover:bg-red-50 transition-all duration-200" title="Delete past event">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div v-else class="flex-shrink-0">
                <button @click="viewEventDetails(event)" class="text-gray-500 hover:text-gray-700 p-2 rounded-lg hover:bg-gray-100 transition-all duration-200">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
